...
const AuthModal = {
    ...
    init() {
        ...
        <div class="amodal" role="dialog" aria-modal="true">
          ...
          <div id="aTabRegister" style="display:none">
            <p class="amodal-sub">Создайте аккаунт в ARTEMIS</p>
            <form id="registerForm" class="amodal-form" novalidate>
              <div class="fg"><label class="fl">Имя</label>
                <input class="fi" type="text" name="name" placeholder="Ваше имя" required></div>
              <div class="fg"><label class="fl">Email</label>
                <input class="fi" type="email" name="email" placeholder="your@email.com" required></div>
              <div class="fg"><label class="fl">Телефон <small>(необязательно)</small></label>
                <input class="fi" type="tel" name="phone" placeholder="+7 (___) ___-__-__"></div>
              <div class="fg"><label class="fl">Пароль</label>
    ...
    async _register(e) {
        ...
        const p = { name: f.name.value.trim(), email: f.email.value.trim(),
                    password: f.password.value, phone: f.phone.value.trim() };
        if (!p.name || !p.email || !p.password) { Toast.error('Ошибка', 'Заполните обязательные поля'); return; }
...
const AuthState = {
    ...
    _sync() {
        ...
        document.querySelectorAll('[data-authed]').forEach(el => el.style.display = u ? '' : 'none');
